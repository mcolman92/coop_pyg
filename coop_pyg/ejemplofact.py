...
class res_company(models.Model):
    ...
    interest_ids = fields.One2many('res.company.interest', 'company_id', 'Interest',)
...
class res_company_interest(models.Model):
    ...
    interest_product_id = fields.Many2one('product.product','Interest Product', required=True, )
    ...
    rule_type = fields.Selection([('daily', 'Day(s)'),('weekly', 'Week(s)'),('monthly', 'Month(s)'),('yearly', 'Year(s)'),],'Recurrency', help="Interests Invoice automatically repeat at specified interval", default='monthly',)
    interval = fields.Integer('Repeat Every', default=1, help="Repeat every (Days/Week/Month/Year)")
    tolerance_interval = fields.Integer('Tolerance', default=1, help="Number of periods of tolerance for dues. 0 = no tolerance")
    next_date = fields.Date('Date of Next Invoice', default=fields.Date.today,)
    ...
    def create_interest_invoices(self):
        ...
        self.next_date = fields.Date.to_string(interests_date_date + next_delta)

    @api.one
    def create_invoices(self, to_date):
        move_line_domain = [('account_id', 'in', self.receivable_account_ids.ids),('reconcile_id', '=', False),('date_maturity', '<', to_date)]
        move_line = self.env['account.move.line']
        grouped_lines = move_line.read_group(domain=move_line_domain,fields=['id', 'debit', 'credit', 'partner_id', 'account_id'], groupby=['partner_id'],)
        for line in grouped_lines:
            ...
            partial_lines = move_line.search(line['__domain'] + [('reconcile_partial_id', '!=', False)])
            # vemos la diferencia entre el saldo total y el conciliado
            reconciled_amount = (sum(partial_lines.mapped('debit')) - sum(partial_lines.mapped('credit')) - sum(partial_lines.mapped('amount_residual')))
            ...
            invoice_vals = self._prepare_interest_invoice(partner, debt, to_date)
            #enviamos tipo de documento para compatibilidad con facturas argentinas
            invoice = self.env['account.invoice'].with_context(document_type='debit_note').create(invoice_vals)
    ...
    def _prepare_interest_invoice(self, partner, debt, to_date, journal=None):
        ...
        if journal is None:
            ...
            if not journal:
                raise Warning(_('Please define sales journal for this company: "%s"') % (company.name))

        comment = _('Deuda Vencida al %s: %s\n''Tasa de interés: %s') % (to_date, debt, self.rate)
        ...
        invoice_vals = {'type': 'out_invoice','account_id': account_id,'partner_id': partner.id,'journal_id': journal.id,'reference': self.interest_product_id.name,'comment': comment,'invoice_line': [(0, 0, self._prepare_interest_invoice_line(partner, debt, to_date))],'currency_id': self.company_id.currency_id.id,
            'payment_term': partner.property_payment_term.id or False,'fiscal_position': partner.property_account_position.id,'date_invoice': self.next_date,'company_id': self.company_id.id,'user_id': partner.user_id.id or False}
        return invoice_vals
    ...
    def _prepare_interest_invoice_line(self, partner, debt, to_date):
        ...
        name = _('%s.\n''Deuda Vencida al %s: %s\n''Tasa de interés: %s') % (self.interest_product_id.name,to_date, debt, self.rate)

        amount = self.rate * debt
        line_data = self.env['account.invoice.line'].with_context(force_company=company.id).product_id_change(self.interest_product_id.id, self.interest_product_id.uom_id.id, qty=1.0, name='', type='out_invoice', partner_id=partner.id, fposition_id=partner.property_account_position.id, company_id=company.id)

        account_id = line_data['value'].get('account_id')
        if not account_id:
            prop = self.env['ir.property'].with_context(force_company=company.id).get('property_account_income_categ','product.category')
            prop_id = prop and prop.id or False
            account_id = self.fiscal_position.map_account(prop_id)
            if not account_id:
                raise Warning(_('There is no income account defined as global ''property.'))
        line_vals = {'product_id': self.interest_product_id.id, 'name': name, 'account_analytic_id': self.analytic_account_id.id, 'price_unit': amount,'quantity': 1.0,'account_id': account_id,'invoice_line_tax_id': [(6, 0, line_data['value'].get('invoice_line_tax_id', []))],}
        return line_vals
...
################################ C R E A R   F A C T U R A #####################################
    ...
    def create_invoice(self):
        interests_date = self.next_date
        interests_date_date = fields.Date.from_string(interests_date)
        to_date = fields.Date.to_string(interests_date_date)
    ...
    def create_invoices(self):
        ...
        partner = self.env['res.partner'].browse(partner_id)
        invoice_vals = self._prepare_interest_invoice(partner)
        #enviamos tipo de documento para compatibilidad con facturas argentinas
    ...
    @api.multi
    def _prepare_interest_invoice(self, partner):
        self.ensure_one()
        
        account_id = self.invoice_receivable_account_id.id
       
        invoice_vals = {'type': 'out_invoice','account_id': account_id,'partner_id': partner.id,'reference': self.product_fee.name, 'invoice_line': [(0, 0, self._prepare_interest_invoice_line(partner))], 'currency_id': self.company_id.currency_id.id, 'payment_term': partner.property_payment_term.id or False, 'fiscal_position': partner.property_account_position.id, 'date_invoice': self.next_date, 'company_id': self.company_id.id, 'user_id': partner.user_id.id or False}
        return invoice_vals
    ...
    @api.multi
    def _prepare_interest_invoice_line(self, partner):
        self.ensure_one()
        company = self.company_id
        name = _('%s.\n''Cuota de ingreso nvo socio ') % (self.product_fee.name)
        amount = self.fixed_fee
        line_data = self.env['account.invoice.line'].with_context(force_company=company.id).product_id_change(self.product_fee.id, self.product_fee.uom_id.id, qty=1.0, name='', type='out_invoice', partner_id=partner.id, fposition_id=partner.property_account_position.id, company_id=company.id)

       
        line_vals = {'product_id': self.product_fee.id, 'name': name, 'price_unit': amount,'quantity': 1.0,'invoice_line_tax_id': [(6, 0, line_data['value'].get('invoice_line_tax_id', []))],}
        return line_vals